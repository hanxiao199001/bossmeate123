/**
 * ⚠️ 勿直接跑（7-25 交接提示）：这是手动运维脚本，不在任何定时任务里，全量跑会持续外部抓取。
 * ⚠️ 跑前先确认现状：万方页面结构/反爬随时会变（resolver 的搜索页链接格式最后一次实抓校准是 7-16，
 *    之后未复验）；先 dry-run（默认）+ --limit=30 看命中率，再决定 --apply；且必须加 --medical-only
 *    （万方医学网只收医学刊，非医学刊实测 0 命中 = 白抓招封）。
 * ⚠️ 爬取节奏铁律：绝不与 LetPub 富化 / 期刊列表爬同时跑（同源 IP 叠加请求 = 被封）。
 *
