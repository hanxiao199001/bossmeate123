...
export async function agentAdminRoutes(app: FastifyInstance) {
  ...
  app.post("/agent-admin/client-package", async (request, reply) => {
    ...
    // 启动器(按系统) + 说明
    const launchers = platform === "windows" ? ["start-agent.bat", "stop-agent.bat", "清理旧客户端.bat"]
      : platform === "mac" ? ["start-agent.command", "停止常驻.command", "清理旧客户端.command"]
      : ["start-agent.command", "start-agent.bat", "停止常驻.command", "stop-agent.bat", "清理旧客户端.bat"];
