...
import { fetchJournalCoverFromLetPub } from "../services/crawler/journal-image-crawler.js";
...
请直接输出文章内容（以标题开头）：`;
    ...
    try {
      ...
      // === 期刊封面图抓取（数据卡片已移除） ===
      const journalImages: Array<{ name: string; coverUrl: string | null }> = [];
      if (journals.length > 0) {
        for (const j of journals) {
          let coverUrl: string | null = null;
          try {
            coverUrl = await fetchJournalCoverFromLetPub(j.name);
          } catch {
            // 封面抓取失败不阻塞
          }
          journalImages.push({ name: j.name, coverUrl });
          // 限速
        ...
        // 在文章中每个期刊首次出现的段落后插入封面图
        const coversToInsert = journalImages.filter((ji) => ji.coverUrl);
        if (coversToInsert.length > 0) {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            for (const ji of coversToInsert) {
              ...
              newLines.push("");
              newLines.push(`![${ji.name}封面](${ji.coverUrl})`);
              newLines.push("");
          ...
          logger.info({ inserted: insertedJournals.size }, "工作流：期刊封面图注入完成");
        }
      }
