/**
 * ═══ 🔴 8-23 补：预注册期望值的第二个作用 —— 它是**停止条件** ═══
 *
 * 我们一直说预注册是为了「防止事后编解释」。8-23 修标题-正文判据那次，
 * 它多了一个作用，而且这个作用当场救了三轮迭代：
 *
 * ```
 * 命中数   358 → 190 → 67 → 55 → 55 → 54
 *                    ↑
 *          第二轮：降幅 81%，任何人都会觉得「差不多了」
 * ```
 *
 * 而实际上后面还藏着**三种**误伤形态（间隔里夹「不」、否定字集缺「没」、
 * 另一个入口 `上了?预警`）。让人没在第二轮停下的，只有一样东西：
 * 跑之前写下的那句「**预期约 43，如果不是 43 附近说明还有第三种误伤形态**」。
 *
 * ▎ 没有预期值，任何一个看起来合理的数字都可以是终点 ——
 * ▎ 而「看起来合理」恰恰是收敛不足时最常见的样子。
 *
 * 所以注册判据时**必须同时注册期望值**，哪怕它只是个粗估：
 * 它不只是事后对账的基准，更是让人知道**什么时候还没做完**。
 *
 * 一个没有期望值的验证，只能回答「变好了吗」，回答不了「够了吗」。
 */
