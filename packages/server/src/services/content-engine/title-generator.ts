...
export async function generateTitles(opts: {
  ...
  if (tone === "strict") {
    ...
    if (banned.length > 0) {
      /**
       * 🔴 8-23：限额用完时**必须给出替代技法**，不能只说「换其它卖点表述」。
       *
       * ## 为什么这是阻塞前置条件
       *
       * 六维 `topicHook` 判据认的是五种具体技法（痛点场景切入 / 开放式提问 /
       * 真实数据反差 / 身份代入 / 悬念前置），并且写死「平铺直叙介绍背景 ≤5分」。
       *
       * 而原文案只说「换其它卖点表述」—— **判据知道五种钩子，生成侧不知道。**
       * 模型最省力的猜法就是退回平铺直叙，正好撞上那条硬上限。
       *
       * 于是限量一收紧，topicHook 会掉 —— 而那个掉分**不是内容变差，是我们没告诉它怎么写**。
       *
       * 同一形态今天第二次（前一次：判据要求「每 200 字 1 个硬数据」，
       * 而 roundup 体裁一篇说 3 本刊、天然做不到）。都是**判据知道标准、生成侧不知道**。
       *
       * ⚠️ 这五种技法的措辞**照抄 `quality-check-v2` 的 topicHook 判据原文**。
       * 两处分开维护必然漂移（红线：判据必须有唯一归宿）——
       * 日后判据改了技法列表，这里要跟着改，注释里留了指路。
       */
      toneSuffix = `\n\n【措辞轮换】以下夸张措辞在本号最近的标题里已用满, 本次标题严禁出现: ${banned.join("、")}。\n` +
        `**换成下面任一钩子技法**(评分器认这五种, 任一到位即算满分):\n` +
        `① 痛点场景切入 —— 直接点出读者当下的困境(如"审稿三个月没消息?")\n` +
        `② 开放式提问 —— 用问题把读者拉进来(如"这本刊到底适合谁投?")\n` +
        `③ 真实数据反差 —— 用两个真数字制造张力(如"IF 3.2, 审稿只要 6 周")\n` +
        `④ 身份代入 —— 点名读者身份(如"青椒评职称""硕博毕业党")\n` +
        `⑤ 悬念前置 —— 先给结论一半, 留一半在正文\n` +
        `🚫 **绝对不要退回平铺直叙介绍背景** —— 那在评分器里是 ≤5 分的封顶项, 比用狠话还糟。\n` +
        `其余规则不变, 数字仍要真实。`;
    }
  }
