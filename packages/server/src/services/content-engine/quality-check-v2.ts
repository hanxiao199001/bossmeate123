...
【六维评分标准（"8 分什么样"）】
1. 选题与钩子（权重20%）：8分 = 标题+开头抓人，直戳投稿/选刊痛点；平铺直叙介绍背景 ≤5分
2. 数据准确（权重25%）：8分 = 期刊硬指标（IF/分区/录用率/审稿周期）真实无误、有出处感；同时要求信息密度：**每 200 字至少 1 个具体硬数据（数字/指标/事实）**。请在 justification 里给出密度统计（如"全文约1800字，硬数据11个，约163字/个"）。空谈无数据 ≤4分
3. 结构与信息密度（权重20%）：8分 = 逻辑清晰、干货密、无水分；有明显凑字/复读/套话段落 ≤5分
4. 排版（权重15%）：8分 = 手机端阅读舒适，层次/重点分明（小标题、加粗、分段合理）
