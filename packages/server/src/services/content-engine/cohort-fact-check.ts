 ...
 *
 * ## 🔻 已降级为**影子检查**（8-11）—— 只记录，不计入违规数、不进对比页
 *
 * 台账：约 30 篇实测产物上累计报出 **37 条，真阳性 0 条**，为此收窄了三次。
 * 按项目自己的判据，这就是一个**零判别力检查器**；再让它计入拍板指标，
 * 等于用噪声污染决策数字。
 *
 * **为什么不删**：它防的失败模式理论上真实存在（「本刊是北大核心期刊」会随目录更新变假），
 * 只是当前 prompt 侧的措辞纪律一直有效（模型稳定写「入选 X 版目录」）。
 * 一旦 prompt 退化或换模型，它是第一道预警。代码与测试全部保留。
 *
 * **重新升级为闸的条件**：连续出现**真阳性 ≥ 2 例**（人工确认过的、确实会随目录更新变假的句子）。
 * 在那之前它只写 metadata。
 *
 * 📌 这条处置是"判据自检"的人肉预演：一个检查器上线后应当自己积累真/假阳性台账，
 *   台账烂就自动降级 —— 而不是靠人偶然想起来去数它报了多少条、中了几条。
 */
