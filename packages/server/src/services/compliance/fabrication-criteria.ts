 ...
 *
 * ## 排除项经 8-10 全量实测确定，别凭直觉再加
 *
 * 「X区」是裸匹配，会撞上「这一**区别**」「单一**区域**」这类普通词。
 * 在 1861 篇存量正文（7685 次命中）上逐字统计后一个字：
 *
 *   后跟「别」 **0** 次 ｜ 后跟「域」 **1** 次 ｜ 后跟「分」 **25** 次
 *
 * → 只排除 `别` 与 `域`。**「分」绝不能排** —— 那 25 次是
 *   「工程技术3区分类」「生物学1区分区」，全是**真分区断言**；
 *   排掉它等于往编造闸里加假阴性，方向比假阳性糟得多。
 *
 * 存量冲击面本来就是 0 篇（唯一那次「单一区域」所在文章另有真分区句，闸照样响）。
 * 改它是为了新体裁——「学科定位」全文禁分区，一个「这一区别」就是唯一命中。
 */
export const TITLE_PARTITION_CLAIM = /(?:中科院\s*|JCR\s*)?(?:Q\s*[1-4]|[1-4一二三四]\s*区(?![别域]))/gi;
