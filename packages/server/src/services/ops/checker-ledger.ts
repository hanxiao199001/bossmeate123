...
// ══════════════════════════════════════════════════════════════════
//
// 🔴 **新增任何「按比率」的判定，一律用「覆盖内容数」做分母，不许用 evaluated。**
//
// evaluated 是**运行次数**：draft-distributor 每天重扫存量草稿，
// 8-16 实测 evaluated=841 而同期出稿 191 篇，重扫系数 k≈4.4。
// 用它做分母，判定会随积压量和重扫轮数漂移 —— 同一个闸，
// 昨天不触发今天触发，而闸本身一个字没改。
//
// 现有两条规则的实测结论（见 __tests__/checker-ledger-denominator-inflation.test.ts）：
//   · 降级规则：分母无关，但**分子 hits 同样被重扫放大** ——
//     「累计报 20 条」可能只是 5 篇被扫了 4 遍。不等于免疫。
//   · 常数检测：**不免疫**。只在存量上恒真的闸，按次数算 95.9% 会触发，
//     按篇数算 85.3% 不该触发；重扫轮数越多越容易触发。
//     方向是更敏感而非失效，且判定只是 warn（提工单），所以暂不改判据 ——
//     但覆盖数列补上之后，这条要第一个改。
...
export function judge(s: CheckerStats): CheckerVerdict {
   ...
   * 看的不是命中数，是触发条件在现实中可不可达。**（老韩 8-20 立）
   *
   * 同一天的两个实例正好凑成一对：
   * ```
   * draft_only 能力过滤         0 条命中 → **保留**：那个号真实存在，
   *                                       走一次批量分发就会命中
   * published_by_operator 探测  0 行     → **死的**：依赖的 wechat_stats 表
   *                                       根本没建过，触发条件不可达
   * ```
   * 命中数都是 0，但一个在等**会发生**的事，另一个在等**不可能发生**的事。
   * 只看命中数它们长得一模一样，而处置完全相反（一个留着，一个是要删的死代码）。
   *
   * 所以这条给的不是「确认场景是否还存在」这种含糊话，而是一个可回答的问题：
   * **触发它需要什么？那件事现在能不能发生？**
   */
  ...
  if (s.hits === 0 && s.evaluated >= 200) {
    return {
      ...
      //   ("这不是坏消息"), 不是一个要人去做的动作。
      // ⚠️ 写「次」不写「篇」, 并点明重扫 —— evaluated 是运行次数,
      //   存量草稿每天重过一遍闸, 8-16 实测是出稿篇数的 4.4 倍。
      message:
        `评估 ${s.evaluated} 次零命中（次数含存量草稿每日重扫，不等于内容篇数）。` +
        `判它死活**别看命中数，看触发条件在现实中可不可达**：它依赖的表/字段/外部接口` +
        `是否存在且有数据。可达 = 正确地安静；不可达 = 死代码。`,
      action: null,
    };
  }
