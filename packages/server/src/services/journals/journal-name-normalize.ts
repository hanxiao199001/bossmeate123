...
export function normName(name: string): string {
  ...
  // 8-11: 汉字之间的点号统一成间隔号。CSSCI 官方表用「课程·教材·教法」,
  //   而入库侧存的是「课程.教材.教法」—— 不统一, 这本刊永远匹配不上官方目录,
  //   下游就会对一本实际入选 CSSCI 的刊断言"未收录"。
  //   ⚠️ 必须放在上面那条 ".XX版" 规则**之后**: 「西南师范大学学报.自然科学版」
  //     要先被转成括号形式, 否则会被误改成「学报·自然科学版」。
  s = s.replace(/\.(?=[一-龥])/g, "·");
  return s;
}
