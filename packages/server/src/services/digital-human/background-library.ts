...
/**
 * 🔴 案子存档：带背景的数字人视频，从 7-31 到 8-18 一次都没成功过（8-18）
 *
 * ```
 * 图库存量                 1 张，就是那张 1600×2848（7-31 上传）
 * 不合规率                 1/1 = 100%
 * 带背景的尝试             3 次（8-13 / 8-14 / 8-18），3 次全被闸拦下，0 成功
 * 同期 video 整体          13 条 / 10 条有产物 = 77%
 * ```
 *
 * 8-13 的修复是**上传侧归一**（新图自动转成 1080×1920）；7-31 那张是存量，从未处理。
 * 而图库当时只有这一张 —— 所以「带背景」这条路从功能上线起就没通过。
 *
 * ## 为什么拖了这么久没人发现
 *
 * **产出没有归零。** 不带背景的视频照常出片，整体成功率 77% 看起来完全正常；
 * 闸每次都正确工作（扣费前中止、零扣费、落 incident、内容标 failed），
 * **正确到没人意识到「带背景」这个分支的产出是 0**。
 *
 * > **闸的告警只说明它在工作，不说明业务在工作。**
 * > 要单独看**成功率**，不能只看**失败告警** —— 一个每次都被正确拦下的输入，
 * > 在告警侧表现为"系统健康"，在业务侧表现为"这个功能不存在"。
 *
 * 这是「止血成功但没治病」的完整形态：闸把损失从「扣费 + 废片」降到「零扣费 + 空壳」，
 * 代价是让问题变得安静。所以修法不是把闸调松，是让不合规的输入
 * **选不出来**（见 `isBackgroundUsableForGeneration`），并且验收标准是**产出恢复**，
 * 不是告警消失。
 */
