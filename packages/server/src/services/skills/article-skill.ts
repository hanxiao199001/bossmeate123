...
7. 【结构·别凑字】(8-23 补: 判据里有、prompt 里原来没有) 评分器的"结构与信息密度"维只看**组织方式**: 逻辑清晰、层次分明、**无凑字/复读/套话段落**。同一件事换个说法再讲一遍 = 复读; "随着学术竞争日益激烈"这类不含信息的过渡 = 套话; 两者出现明显段落即封顶 5 分。宁可少写一段, 不要注水。
8. 【实用性·读完能做成一件事】(8-23 补: 同上) 评分器的"实用性"维要的是**读者合上页面就知道下一步动手做什么, 且做得到** —— 不限定是投稿动作: 可以是"照这个口径筛一遍候选""拿这份清单对照自己的稿子""先去官网确认某一项"。只给方向不给做法("要重视选题"这类) = 5 分; 通篇只有描述、读完不产生任何可做的事 ≤3 分。结尾的行动号召(CTA)**不等于**实用性 —— CTA 是让读者关注/私信, 实用性是让读者能动手。
`;
