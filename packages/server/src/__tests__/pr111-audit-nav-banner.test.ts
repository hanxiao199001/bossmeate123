...
describe("PR #111: 期刊审计入口（admin only） — 5-21 P0 从 DashboardPage ToolGrid 搬到 Sidebar", () => {
  ...
  it("非 admin 时不渲染该入口（声明式 adminOnly 过滤）", () => {
    // 7-08 更新(测试过时/实现演进): 隐藏机制从旧 `isAdmin ? [...]` 数组 spread 改为
    //   nav item 声明 `adminOnly: true` + 统一过滤 `if (i.adminOnly && !isAdmin) return false`。功能不变(期刊审计入口仍 admin-only)。
    expect(src).toMatch(/to:\s*"\/admin\/journals\/audit"[\s\S]{0,120}adminOnly:\s*true/);
    expect(src).toMatch(/i\.adminOnly\s*&&\s*!isAdmin/);
  });
});
