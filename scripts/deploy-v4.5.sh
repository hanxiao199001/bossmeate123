...
# ============================================
# BossMate V4.5 部署脚本
# V4 基础 + SkillRegistry + Task系统 + 内容引擎 + 数据采集
# ============================================
...
TAR_FILE="/tmp/bossmate-v4.5-deploy.tar.gz"

echo "🚀 BossMate V4.5 部署（需扫2次码）"
echo ""
...
tar czf "$TAR_FILE" \
  ...
  packages/server/src/routes/content-engine.ts \
  packages/server/src/routes/data-collection.ts \
  \
  ...
  packages/server/src/services/knowledge/rag-retriever-v2.ts \
  ...
  \
  packages/server/src/services/skills/base-skill.ts \
  packages/server/src/services/skills/skill-registry.ts \
  packages/server/src/services/skills/index.ts \
  ...
  packages/server/src/services/task/content-worker.ts \
  packages/server/src/services/task/progress-ws.ts \
  \
  packages/server/src/services/content-engine/article-pipeline.ts \
  packages/server/src/services/content-engine/content-calendar.ts \
  packages/server/src/services/content-engine/content-repurpose.ts \
  packages/server/src/services/content-engine/format-generators.ts \
  packages/server/src/services/content-engine/quality-check-v2.ts \
  \
  packages/server/src/services/data-collection/hot-event-timeline.ts \
  packages/server/src/services/data-collection/domain-knowledge-collector.ts \
  packages/server/src/services/data-collection/competitor-analyzer.ts \
  packages/server/src/services/data-collection/hot-event-monitor.ts \
  packages/server/src/services/data-collection/style-learning-enhanced.ts \
  packages/server/src/services/data-collection/ingest-pipeline.ts \
  packages/server/src/services/data-collection/data-structurizer.ts \
  packages/server/src/services/data-collection/quality-check-engine.ts \
  \
...
scp "$TAR_FILE" "$SERVER:/tmp/bossmate-v4.5-deploy.tar.gz"
echo "   上传完成！"
...
echo ">>> 解压 V4.5 文件..."
tar xzf /tmp/bossmate-v4.5-deploy.tar.gz
...
if redis-cli ping 2>/dev/null | grep -q PONG; then
  ...
  redis-server --daemonize yes 2>/dev/null || echo "   ❌ Redis 启动失败，Task 队列功能将不可用"
fi
...
rm -f /tmp/bossmate-v4.5-deploy.tar.gz

echo ""
echo "✅ V4.5 部署完成！"
echo ""
echo "V4.5 新增功能："
echo "  - SkillRegistry 技能注册中心（可插拔技能系统）"
echo "  - BullMQ Task 异步任务系统 + WebSocket 进度推送"
echo "  - 内容引擎（文章流水线 / 内容日历 / 内容改写 / 质量检查V2）"
echo "  - 数据采集（热点监控 / 竞品分析 / 领域知识采集 / 风格学习增强）"
echo "  - RAG 检索 V2"
REMOTE_SCRIPT

echo ""
echo "🎉 V4.5 部署全部完成! 访问: http://106.53.163.120"
