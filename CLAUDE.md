...
## 红线 #25 — 测不到的东西要在运行时可见，不要写一个测不到的测试假装覆盖了它（2026-08-23 老韩立）

**强制规则**：当某件事**单测原理上测不到**（真库行为、外部 API、真实网络、时序），
正确处置是**让它在运行时可见**（失败落 incident / 指标 / 简报），
**不是**写一个 mock 掉真实依赖的测试来"覆盖"它。

> **一个 mock 掉真实依赖的测试，覆盖的是 mock，不是代码。**
> 它给出的绿色**比没有测试更危险** —— 因为没有测试时，人还会手动验一次。

### 病历（8-23）

```
reportStarvedAccounts 的 SQL 用 `= ANY(${array})`
  → drizzle 绑不成 Postgres 数组，从写下来那天就是坏的
  → 而我给它加的守卫把 db.execute mock 成"返回现成的行"
  → 测试绿。真 SQL 一次没跑过。
```

**mock 掉的正好就是坏掉的那一部分。** 而那条测试的存在，让人觉得这块已经有覆盖了。

### 正确的三件套

| 测不到的东西 | 该怎么办 |
|---|---|
| SQL 会不会执行 | 真库跑一次（红线 #19），**不是**单测 |
| 检查器挂了会不会被发现 | catch 里落 incident（红线 #23）—— **这个能单测** |
| 这段代码有没有真跑过 | 断言副作用（红线 #24）+ 真环境 smoke |

所以本次补的守卫**只锁失败可见性**，不假装能锁 SQL 正确性 ——
判据要诚实地只覆盖它真能覆盖的那部分。
...
|---|---|
| **#24** | 测试要证明代码**真的跑了** |
| **#25** | 测不了就**别假装测**，改成运行时可见 |

### 推论：每个 mock 都是一次「这段代码不再被验证」的决定

mock 掉 `db.execute` / `fetch` / 动态 import 时，问一句：
**被它挡住的那段逻辑，还有谁在验？** 答案是"没有"的话，那就得有一个运行时的眼睛。

---

## 红线 #24 — 测试通过 ≠ 代码执行过（2026-08-23 老韩立）

**强制规则**：断言**副作用**，不要断言「没报错」。

```ts
✗ expect(() => report()).not.toThrow()
✓ expect(insertedRows).toHaveLength(1)      // ← 证明它真的写进去了
✓ expect(items[0].text).toContain("9/100")  // ← 证明它真的算出来了
```

### 病历（8-23，一天之内两处 harness 同病）

```
db mock 没有 execute
  → reportStarvedAccounts 用的是原生 SQL，一调就抛
  → 被它自己的 try/catch 吞掉
  → 测试全绿，而这条告警从来没有执行过一次
```

`ops-daily-briefing.test.ts` 的 harness 是同一个洞。两处都是：
**一个被 mock 挡住、被 catch 吞掉的函数，测试会绿到底。**

> **一个从未真正执行过的检查，在所有表面观察下，都和「正常工作的检查」完全一样。**
...
两条防的是同一件事的两半。只有 #23 而没有 #24：报错逻辑写了，但那段代码在测试里从没进过，
所以「它会报错」这句话没有任何证据。

### 怎么落地

- **每个 mock 都是一次「这段代码不再被验证」的决定**。mock 掉 `db.execute` / `fetch` /
  动态 import 时，问一句：被它挡住的那段逻辑，还有谁在验？
- 断言里至少有一条钉住**产物**：落库的行、产出的告警条目、发出去的请求体、返回的结构。
- 只断言「没抛错」「返回了 truthy」「调用次数 ≥1」的测试，等于只测了「代码没炸」，
  没测「代码干了它该干的事」。
...
基线闸提取器      → 自己 fail-open        （#23）
长期零供给告警    → 内部对了，外层不执行  （调用条件不得依赖「今天有没有别的问题」）
本条              → 测试绿，函数从没跑过  （#24）
```

**五次都是同一句话：一个不工作的检查，长得和工作的检查一模一样。**

---

## 红线 #23 — 检查器上线前必须回答「你自己挂了会怎样？」（2026-08-22 老韩立）

**强制规则**：任何检查器 / 判据 / 守卫 / 闸上线前，必须回答一个问题 ——

> **「你自己挂了会怎样？」**
> 答案只能是「**报错 / 标记不可用**」，**不能**是「当作通过」。

**这是红线 #14 在判断层的形态**：检查器的失败**也是一种降级产物**，
同样必须可区分于「检查通过」。#14 管的是产物侧（兜底文案不许长得像真数据），
#23 管的是判断侧（检查器挂了不许长得像检查通过）。

### 8-22 一天之内同一形态两次

| | 检查器自己失败时 | 下游看到的 |
|---|---|---|
| 六维评分器 | JSON 截断、尾部维度缺失 | **一个数字**（16 分）——与「内容真差」不可区分 |
| CI 基线闸 | 测试文件 import 期就挂 | **零条失败**——与「这个文件全绿」不可区分 |

两次都是「**检查器自己失败时，静默通过**」。

**成因也一样**：注意力全在「要检查什么」上，没有人问「检查器自己挂了会怎样」。
写判据的时候，脑子里的画面永远是「判据在工作、看着被检查的东西」，
从来不是「判据自己躺下了」。

第二次尤其值得记：那是一道**专门为了防 fail-open 而建的闸，它自己是 fail-open 的**。
所以这条规矩不能靠「注意一点」——必须变成上线前的一道固定提问。

### 怎么算答过了

不是嘴上答，要能指出**代码里的哪一行**让「检查器挂了」走向报错而不是通过：

- ✅ 六维缺维 → `throw`，走既有重打/降级链，最终判 `quality_check_unavailable`（**没评上分 ≠ 评了 0 分**）
- ✅ 撞顶 `finishReason==="max_tokens"` → 解析之前就拒收
- ✅ CI 基线闸 → `status==="failed" && assertionResults.length===0` 单独捞成「套件加载失败」
- ✅ 配套：总用例数写进基线文件头，变少时告警（#12 规则 6 那个点就是为这一刻打的 —— 8-22 它当场抓住了写闸的人自己）

### 顺带：检查器的输出要有信息量（同族，来自同一天）

> **恒红的检查器 = 命中率 100% = 零判别力。**

CI 恒红两个半月给不出任何信息，不是因为它没跑，是因为它的输出里没有信息量
（Phase 1 判定规则第一条，本来是写给内容检查器的，对 CI 自己同样成立）。
而它比没有检查器更坏 —— 它的存在让每个人以为「有闸在把关」。

**判据的价值在于它区分了什么，不在于它是否运行。**

---

## 红线 #22 — 参数外化之后：「部署了新默认值」≠「新默认值生效了」（2026-08-18）

**强制规则**：参数类改动的 verify **必须读配置表的实际生效值 + 来源**。
不看代码，不看 env。

### 为什么

三层读取顺序 `DB → env → 代码默认` 意味着：

```
代码里改默认值  →  只对【从未被配置过】的参数生效
DB 里任何残留    →  静默盖住新默认（包括自测写进去的）
而代码里白纸黑字写着新值 →  看代码的人会以为它生效了
```

**实测（8-18）**：

```
代码默认  WATCHDOG_TIMEOUT_MS = 40 分
实际生效  getParam            = 30 分     ← DB 里 30 分钟前的自测残留
参数页    显示 30 [来源: 配置]
```

如果按「看代码 = 40，收工」，线上会**继续以 30 分钟运行** ——
而 30 < 心跳最坏间隔的 3 倍（36），理论上仍会误杀，且**没人会发现**，
因为代码里写着 40。

### 配套两条

**① 自测写配置必须当场清理**，或用独立的测试参数名（不碰生产 key）。
清理时**同样落审计行** —— 删除也是一次变更，「谁把它删了」和「谁把它改了」一样要能查。

**② 参数页必须显示「当前值 + 来源（配置/环境变量/默认）」。**
运营看到的必须是**生效值**，不是任何一层的**声明值**。
只显示数字而不显示来源，就无法回答「这个 30 是配的还是默认的」——
而这两种情况的处置完全不同（前者要改配置，后者要改代码）。

### 与 #19 / #20 同族：声明的状态 ≠ 实际的状态

| 红线 | 声明 | 实际 | 靠什么发现 |
|---|---|---|---|
| #19 | tsc/单测/守卫全绿 | 真库 42P10 | 在真库上跑冲突路径 |
| #20 | `length(body)` = 11027 | 净正文 869 | 看那个量的真实分布 |
| #22 | 代码默认 40 | 实际生效 30 | 读配置表的生效值 + 来源 |

> 三条都是同一件事：**你以为的状态，和系统真正处在的状态，中间隔着一层没人去看的东西。**
...
| 案例 | 口径 A | 口径 B | 混用后果 |
|---|---|---|---|
| `sustainable` 当投放判据 | 「回补 ≥ 消耗」= **永续性** | 「够不够用」= **可投放性** | 能撑 55 年的 medicine（1143 本）被判 false 排除，从没人用过的 engineering 被判 true 选中。默认集漏掉最大的三个池子 |
| 「保底」两个层面 | 分发侧：每号 ≥2 篇**进草稿箱** | 生成侧：每号在**自己 scope 内** ≥2 篇 | 两个数可能不等；按学科汇总会丢掉 scope 约束（给 domestic 号 4 篇国际刊内容，它一篇也用不上） |

> **最通用的一条**（8-19 提炼）：
> **没有证据说明它不行，不等于有证据说明它不行。**
> 「明确可持续」与「明确不可持续」这两个集合**不互补** —— 中间隔着一大批
> 「还没数据」的，而它们往往恰恰是最大的那部分。
> 所以判据要写成「**排除明确不行的**」，不是「只要明确行的」。
...
| 22 | 参数生效要读配置表 | 「部署了新默认值」≠「新默认值生效了」：`DB → env → 代码默认` 三层，DB 残留会**静默盖住**新默认，而代码里写着新值。8-18 实测代码 40 / 实际 30（自测残留）。verify 必须读**实际生效值 + 来源**；自测写配置当场清理（删除也落审计）；参数页显示「当前值 + 来源」。见上方 |
| 23 | 检查器挂了不许当通过 | 任何检查器/判据/守卫上线前必须回答「你自己挂了会怎样」，答案只能是**报错/标记不可用**。这是 #14 在**判断层**的形态：检查器的失败也是降级产物，必须可区分于「检查通过」。8-22 一天两次：六维截断→返回一个数字、CI 提取器套件加载失败→零条失败（**那是一道专门防 fail-open 的闸，自己 fail-open**）。同族推论：**恒红的检查器 = 命中率 100% = 零判别力**，比没有更坏。见上方 |
| 24 | 测试通过 ≠ 代码执行过 | 断言**副作用**，不要断言「没报错」。8-23 两处 harness 同病：db mock 没有 `execute` → 原生 SQL 一调就抛 → 被自己的 catch 吞掉 → **测试全绿而那条告警从没执行过**。与 #23 成对（#23：检查器挂了必须报错；#24：测试必须证明它真跑了）。每个 mock 都是一次「这段代码不再被验证」的决定。见上方 |
| 25 | 测不到就别假装测 | 单测原理上测不到的（真库行为/外部 API/时序），正确处置是**让它运行时可见**（失败落 incident/指标），不是写个 mock 掉真实依赖的测试假装覆盖。**mock 掉真实依赖的测试覆盖的是 mock 不是代码；它给的绿色比没有测试更危险 —— 没有测试时人还会手动验一次**。8-23 实例：守卫把 `db.execute` mock 成返回现成的行，而坏的正是那条真 SQL。与 #24 成对。见上方 |
...
### 🔴 CC 回报纪律：每次状态回报第一行先报备份（2026-09-04 老韩立）

> **每次状态回报第一行：最近一次备份成功距今 N 小时 / 近 24h `backup_failed` N 次。
> 超过 30 小时无成功备份时，其他内容之前先报这条。**

**为什么是纪律而不是告警**：告警早就有了，而且工作得很好 ——
`backup_failed` 每次都落 incident，每日简报连报两天、alert 级、`pushed=true`。

```
09-01 02:00  success        ← 唯一一次成功
09-03 02:03  failed
09-04 02:01/02/03  failed
```

**生产连续两天没有备份，而这两天里 CC 做过两次"生产状态回报"，都没查它。**
系统全程在喊，没人动 —— 包括 CC 自己。

▎ 这不是"再加一个告警"能解决的问题（那只会多一个没人看的告警）。
▎ 是把它**挪到必看的位置** —— 回报的第一行，不是靠记得。

（同源：《沉默检查器盘点》治「检查器没工作」，这条治「检查器工作了但输出没人读」。）

### 🔴 判"有没有新增失败"以 CI 为准，不以本地全量为准（2026-09-04 立）

本仓库有 6 个测试文件（16 条用例）**本地全量跑必红、CI 上全绿** —— 已在干净的 `main`
上验证过，不是任何一个 PR 引入的。它们都是"读源文件做结构断言"的守卫，对本地工作区状态敏感。

> **要判回归，跑 `bash scripts/ci/baseline-gate.sh`，或直接看 PR 的 CI。**
> `npx vitest run` 的全量结果在这里读不出"新增失败"这个信号。

9-04 件 2 时踩过：本地 gate 报"新增 6 条"全是这批噪音，而 CI 报的 14 条才是真回归 ——
**两边指的不是一回事，而它们长得一模一样（都是"新增失败 N 条"）。**
清单与背景见 `CC-待办.md` 第 6b 条。

所以每次改动的 verify，除了 mtime/字面/health 三项，再加一行：
**列出预期会动的现有指标 + 预期方向**，上线后当天对一次。
对不上（方向反了、或该动的没动）本身就是信号。

### 🔴 影子模式转正式生效：先清点影子分支里还挂着什么（2026-08-24 老韩立）

> **切换影子模式到正式生效时，必须先列出影子分支里还挂着什么。**
> 影子代码不只是"对比逻辑"，它经常顺带承载了**观测和告警** ——
> 而切换的动作会把它们一起删掉，不留痕迹。

**病历（8-19 → 8-20，配额 v2）**：8-19 上线的影子块里除了 `logShadowComparison`，
还挂着一条 `quota_floor_exceeds_pool` 告警（"教育学：需 2 篇，冷却外可选 0 本"，
它的注释写着「这条会每天喊，直到扩池或改定位解决它」）。8-20 切 v2 的实现是：

```ts
if (await getParam("quota.useV2")) {
  const v2 = await buildQuotaFromV2(aq);
  if (v2) { return v2; }          // ← 提前 return，后面整段影子块（含那条告警）不再执行
}
... void (async () => { /* 影子对比 + quota_floor_exceeds_pool */ })();
```

实测：该 kind **一共只发过 4 条（8-19 两条、8-20 两条），此后归零**，而 `quota.useV2`
的生效时间是 8-20 02:35 —— **一个专门为 v2 建的告警，死于 v2 上线那天**。
它的条件至今仍然成立（国际刊·教育学 fresh=0、need=2）。

**当时验的是「开关生效、判据 pass、generic 兜底率下降」，没有人问过"影子分支里还有别的东西吗"。**

**落到 verify 清单**（与上面那行「现有指标会怎么动」配对使用）：

1. 切换前 `grep` 影子分支的**整个作用域**，列出里面所有 `recordIncident*` / `logger.warn`
   / 指标写入 / 落库动作 —— **不只是对比逻辑**；
2. 逐条判定：这一条在正式路径里**还需不需要**？需要就先搬出来，不需要就删掉并说明；
3. 上线后当天核对：**这些 kind 的发生量应该怎么变**（这正是上一条的应用）。
   一条本该每天响的告警在切换当天归零 —— 那不是"问题解决了"，那是它死了。

> **推论**：这条与红线 #23 同族。#23 说"检查器自己挂了不许当作通过"，
> 这条说的是"**检查器可能死于一次与它无关的重构**" —— 而死法完全无声：
> 没有报错、没有日志、只有一条本该每天出现的记录，从某一天起不再出现。
> 所以「某 kind 的发生量突然归零」必须当成**可疑**，不是当成好消息。

部署目标 = 阿里云新机 `ubuntu@<SERVER_IP>`（可 `export BOSSMATE_DEPLOY_SERVER` 覆盖）。
