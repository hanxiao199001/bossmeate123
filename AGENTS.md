...
| 4 | 云厂商硬约束（已迁阿里云） | 服务器阿里云 ECS **<SERVER_IP>**（key `~/.ssh/bossmate_deploy`，ssh config alias `bossmate-boss`）；存储阿里云 **OSS**（私有桶 + 签名 URL）；LLM 阿里云**百炼**（DeepSeek + Qwen-Plus）；数字人阿里云 **DVH**；语音阿里云 **NLS**；短信阿里云。**腾讯云 COS/CMS/ECS 已全部弃用，<OLD_SERVER_IP> 是旧机（勿再引用）。迁移细节以 `迁移手册-新服务器.md` 为准。** |
...
deploy:smart 路径：直连 fetch 3 次 retry → bundle 绕路兜底（修 PR #49 false-green bug）。部署目标 = 阿里云新机 `ubuntu@<SERVER_IP>`。
