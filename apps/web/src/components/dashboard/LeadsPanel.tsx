...
const STAGE_COLOR: Record<string, string> = {
  ...
  negotiating: "bg-rose-50 text-rose-700",
    ...
    <section className="bg-white rounded-2xl border border-slate-200/70 shadow-[0_1px_2px_rgba(16,24,40,0.04)] p-5 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-1.5">
            <IconRadar size={14} className="text-rose-500" />
            <span>待跟进的客户</span>
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">意向分高 → 优先回</p>
        </div>
        {totalCount > 0 && (
          <span className="px-2 py-0.5 bg-rose-50 text-rose-700 text-xs font-medium rounded-full">{totalCount}</span>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-8">
          <div className="w-12 h-12 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center mb-3"><IconRadar size={20} /></div>
          <p className="text-sm text-slate-500 mb-3">还没有线索来咨询</p>
          <Link to="/sales-radar" className="text-xs font-medium text-indigo-600 hover:text-indigo-500">进销售雷达看历史 →</Link>
        </div>
      ) : (
        <>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm text-slate-900 truncate">{it.name || "匿名线索"}</p>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded ${STAGE_COLOR[it.stage] || "bg-slate-100 text-slate-500"}`}>
